...
!PUN    use SwanGriddata, only: ivertg, nverts, nvertsg
...
!PUN    !
!PUN    call MPI_GATHER( nownv, 1, MPI_INTEGER, icount, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr )
...
!PUN    !
!PUN    call MPI_GATHERV( ivertp, nownv, MPI_INTEGER, iarr, icount, idsplc, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr )
!PUN    if ( ierr == MPI_SUCCESS ) call MPI_GATHERV( blknd, nownv, MPI_REAL, arr, icount, idsplc, MPI_REAL, 0, MPI_COMM_WORLD, ierr )
!PUN    if ( ierr /= MPI_SUCCESS ) then
!PUN       write (msgstr, '(a,i6)') ' MPI produces some internal error - return code is ',ierr
!PUN       call msgerr( 4, trim(msgstr) )
!PUN       return
